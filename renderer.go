...
func (e *Render) Render(out io.Writer, name string, binding any, c echo.Context) (err error) {
	parts, layout := strings.Split(name, ":"), []string{}
	if name = parts[0]; len(parts) > 1 {
		layout = parts[1:]
	}
	...
	if binds, ok := binding.(Map); ok && c != nil {
		if vb := c.Get(ViewBind); vb != nil {
			if global, ok := vb.(Map); ok {
				binding = Merge(global, binds)
			}
		}
	}
